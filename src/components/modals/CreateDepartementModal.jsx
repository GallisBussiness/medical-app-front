...
function CreateDepartementModal({ isOpen, onResolve, onReject }) {

    const qk = ['get_Ufrs']

    const {data: Ufrs} = useQuery(qk, () => getUfrs());

    const defaultValues = {nom: '', ufr: ''};
  ...
  return (
    <>
       <Dialog header="Creer un Departement" visible={isOpen} onHide={() => onReject(false)} className="w-1/2">
...
export default create(CreateDepartementModal);